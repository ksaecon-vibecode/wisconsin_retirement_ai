...
def process_wave(year, zip_path):
    ...
    # Filter: age 25-62
    age_col = next(
        (c for c in ['A3AR_W', 'A3AR', 'A3A_W', 'AGE'] if c in df.columns),
        None
    )
    if age_col:
        df['AGE_NUM'] = pd.to_numeric(df[age_col], errors='coerce')
        df = df[(df['AGE_NUM'] >= 25) & (df['AGE_NUM'] <= 62)].copy()
    attrition['age_25_62'] = len(df)
    log(f"    After age filter (25-62): {len(df):,}")
    ...
    control_map = {
        ...
        'weight_nat':   'WGT_N2' if 'WGT_N2' in df.columns else
                        'wgt_n2' if 'wgt_n2' in df.columns else None,
        'weight_st':    'WGT_S3' if 'WGT_S3' in df.columns else
                        'wgt_s3' if 'wgt_s3' in df.columns else None,
    }
...
if df_hud is not None:
    ...
    if unmatched.sum() > 0:
        ...
        df_hud_any = (
            ...
            .drop_duplicates('zip')
            .rename(columns={'zip': 'zip_clean', 'primary_county_fips': 'county_fips_fb'})
        )
        df_pooled = df_pooled.merge(df_hud_any, on='zip_clean', how='left')
        df_pooled.loc[unmatched & df_pooled['county_fips_fb'].notna(), 'county_fips'] = \
            df_pooled.loc[unmatched & df_pooled['county_fips_fb'].notna(), 'county_fips_fb']
        df_pooled = df_pooled.drop(columns=['county_fips_fb'])

        county_final = df_pooled['county_fips'].notna().sum()
        log(f"  After fallback: {county_final:,} matched ({county_final/len(df_pooled)*100:.1f}%)")
else:
...
df_pooled['race_eth'] = pd.to_numeric(
    df_pooled['race_eth_restricted'], errors='coerce'
)
